...
class EarlyStoppingTest(unittest.TestCase):
  ...
  def test_training_stops_on_patience_lost(self):
     ...
     early_stopping = EarlyStopping(patience=5, min_progress_delta=1e-1, path=self.model_path, enable_logging=False)
     epoch = 0
     while epoch < self.max_epoch and not self.stop_training:
        early_stopping(val_loss[epoch],self.model,self.stop_training_callback)
        epoch += 1
  ...
  def test_early_stopping_logs(self):
     ...
     val_loss = np.linspace(start=10, stop=9.8, num=100, dtype=np.float32)
     early_stopping = EarlyStopping(patience=5, min_progress_delta=1e-1, path=self.model_path, enable_logging=True)
     epoch = 0
     while epoch < self.max_epoch and not self.stop_training:
        early_stopping(val_loss[epoch],self.model,self.stop_training_callback)
        epoch += 1
